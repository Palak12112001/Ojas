...
<div id="content" style="width:100%">
    <div class="content_area">
        <div class="page-title">Bike List</div>
        <ul class="radio-flex">
            <li>
                <label class="radio">
                    <input type="radio" name="wing[]" class="wing" value="A" data-type='bikes' checked>
                    <span class="checkmark">A</span>
                </label>
            </li>
            <li>
                <label class="radio">
                    <input type="radio" name="wing[]" class="wing" value="B" data-type='bikes'>
                    <span class="checkmark">B</span>
                </label>
            </li>
            <li>
                <label class="radio">
                    <input type="radio" name="wing[]" class="wing" value="C" data-type='bikes'>
                    <span class="checkmark">C</span>
                </label>
            </li>
            <li>
                <label class="radio">
                    <input type="radio" name="wing[]" class="wing" value="D" data-type='bikes'>
                    <span class="checkmark">D</span>
                </label>
            </li>
            <li>
                <label class="radio">
                    <input type="radio" name="wing[]" class="wing" value="E" data-type='bikes'>
                    <span class="checkmark">E</span>
                </label>
            </li>
            <li>
                <label class="radio">
                    <input type="radio" name="wing[]" class="wing" value="F" data-type='bikes'>
        ...
        <div class="search-area">
            <div class="inputblock">
                ...
                <button type="button" class="btn-search" name="submit" id="bike_filter"><img src="../assets/images/search-icon.png" alt=""></button>
                <button type="button" class="btn-reset" name="reset" onclick="location.reload();"><img src="../assets/images/reset-icon.png" alt=""></button>
            </div>
        </div>

        <ul class="list-ui wing-result" id="bike_results">
            <?php
            ...
            // Query to fetch data for bikes with two entries and also fetch holder data using flat number
            $query = "SELECT fh.full_name, fh.phone, v.flat_number,
       GROUP_CONCAT(v.bike) AS bikes
FROM tbl_flat_holders fh
JOIN tbl_vehicles v
  ON TRIM(fh.flatno) = TRIM(v.flat_number)
WHERE v.flag = '0'
  AND LEFT(v.flat_number, 1) = 'A'
...
HAVING COUNT(v.bike) = 2";
            ...
            if ($result) {
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    // Check if both bike values are empty
                    if (empty($bikes[0]) && (empty($bikes[1]) || !isset($bikes[1]))) {
                        continue; // Skip this iteration if both bikes are empty
                    }
            ?>
                    <li>
                        <div class="flat-block bike-list">
                            ...
                            if (!empty($bikes[0]) && !empty($bikes[1])) {
                                echo '<div class="bike"><img src="../assets/images/bike.svg" alt="">' . htmlspecialchars($bikes[0]) . '</div><div class="bike"><img src="../assets/images/bike.svg" alt="">' . htmlspecialchars($bikes[1]) . '</div>';
                            } elseif (!empty($bikes[0])) {
                                echo '<div class="bike"><img src="../assets/images/bike.svg" alt="">' . htmlspecialchars($bikes[0]) . '</div>';
                            } elseif (!empty($bikes[1])) {
                                echo '<div class="bike"><img src="../assets/images/bike.svg" alt="">' . htmlspecialchars($bikes[1]) . '</div>';
                            }
                            ?>
                            <a href="tel:+91<?php echo $number; ?>" class="phone">
